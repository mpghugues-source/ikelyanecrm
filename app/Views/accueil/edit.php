...
<form method="POST" action="/admin/accueil/<?= $operation['id'] ?>/update">
    <?= csrf_field() ?>
    <div class="row g-3">
        <div class="col-md-6">
            ...
            <select name="patient_id" class="form-select">
                ...
                <option value="<?= $p['id'] ?>" <?= old('patient_id', $operation['patient_id']) == $p['id'] ? 'selected' : '' ?>><?= esc($p['nom']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-6">
            <label class="form-label"><?= lang('Accueil.visitor') ?></label>
            <input type="text" name="nom_visiteur" class="form-control" value="<?= esc(old('nom_visiteur', $operation['nom_visiteur'])) ?>">
        </div>
        <div class="col-md-4">
            ...
            <select name="type_operation" class="form-select" required>
                ...
                <option value="<?= $val ?>" <?= old('type_operation', $operation['type_operation']) == $val ? 'selected' : '' ?>><?= lang('Accueil.types.' . $val) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-4">
            <label class="form-label"><?= lang('Accueil.datetime') ?> <span class="text-danger">*</span></label>
            <input type="datetime-local" name="date_heure" class="form-control" value="<?= esc(old('date_heure', str_replace(' ', 'T', $operation['date_heure']))) ?>" required>
        </div>
        <div class="col-md-4">
            <label class="form-label"><?= lang('Accueil.room') ?></label>
            <input type="text" name="chambre" class="form-control" value="<?= esc(old('chambre', $operation['chambre'])) ?>">
        </div>
        <div class="col-md-6">
            <label class="form-label"><?= lang('Accueil.status') ?></label>
            <select name="statut" class="form-select">
                <?php foreach (['en_attente','en_cours','termine'] as $val): ?>
                <option value="<?= $val ?>" <?= old('statut', $operation['statut']) == $val ? 'selected' : '' ?>><?= lang('Accueil.statuses.' . $val) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-6">
            <label class="form-label"><?= lang('Accueil.reason') ?></label>
            <input type="text" name="motif" class="form-control" value="<?= esc(old('motif', $operation['motif'])) ?>">
        </div>
        <div class="col-12">
            <label class="form-label"><?= lang('Accueil.notes') ?></label>
            <textarea name="notes" class="form-control" rows="3"><?= esc(old('notes', $operation['notes'])) ?></textarea>
        </div>
