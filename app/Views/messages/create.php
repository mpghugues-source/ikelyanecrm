        ...
        <form method="POST" action="/messages/store">
            ...
            <div class="mb-3">
                ...
                <select name="to_user_id" class="form-select" required>
                    ...
                    <?php
                    $roleLabels = ['admin'=>'Administrateur','secretaire'=>'Secrétaire','medecin'=>'Médecin','patient'=>'Patient'];
                    $grouped = [];
                    ...
                    ?>
                        <optgroup label="<?= $roleLabels[$role] ?? ucfirst($role) ?>s">
            ...
            <div class="mb-4">
                ...
                <textarea name="message" class="form-control" rows="8" required
                    placeholder="Rédigez votre message ici..."><?= esc(old('message')) ?></textarea>
            </div>
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-send me-1"></i>Envoyer
                </button>
                <a href="/messages" class="btn btn-outline-secondary">Annuler</a>
            </div>
        </form>
    </div>
</div>
