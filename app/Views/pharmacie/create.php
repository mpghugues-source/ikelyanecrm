...
<div class="card">
    <div class="card-body">
        <form method="POST" action="/admin/pharmacie/store">
            <?= csrf_field() ?>
            <div class="row g-3">
                <div class="col-md-6">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="categorie" class="form-control" value="<?= esc(old('categorie')) ?>" placeholder="Antibiotique, Antalgique...">
                </div>
                <div class="col-md-3">
                    ...
                    <select name="forme" class="form-select">
                        ...
                        <option value="<?= $f ?>" <?= old('forme') == $f ? 'selected' : '' ?>><?= $f ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label"><?= lang('Pharmacie.dosage') ?></label>
                    <input type="text" name="dosage" class="form-control" value="<?= esc(old('dosage')) ?>" placeholder="500mg, 5ml...">
                </div>
                <div class="col-md-3">
                    <label class="form-label"><?= lang('Pharmacie.current_stock') ?> <span class="text-danger">*</span></label>
                    <input type="number" name="stock_actuel" class="form-control" value="<?= esc(old('stock_actuel', 0)) ?>" min="0" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label"><?= lang('Pharmacie.min_stock') ?> <span class="text-danger">*</span></label>
                    <input type="number" name="stock_minimum" class="form-control" value="<?= esc(old('stock_minimum', 10)) ?>" min="0" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label"><?= lang('Pharmacie.unit_price') ?> (DA) <span class="text-danger">*</span></label>
                    <input type="number" step="0.01" name="prix_unitaire" class="form-control" value="<?= esc(old('prix_unitaire', '0.00')) ?>" min="0" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label"><?= lang('Pharmacie.supplier') ?></label>
                    <input type="text" name="fournisseur" class="form-control" value="<?= esc(old('fournisseur')) ?>">
                </div>
                <div class="col-md-3">
                    <label class="form-label"><?= lang('Pharmacie.expiry_date') ?></label>
                    <input type="date" name="date_expiration" class="form-control" value="<?= esc(old('date_expiration')) ?>">
                </div>
                <div class="col-12">
                ...
                <div class="col-12 d-flex gap-2">
                    <button type="submit" class="btn btn-success"><i class="bi bi-check-lg me-1"></i><?= lang('Common.save') ?></button>
