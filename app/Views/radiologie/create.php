...
<form method="POST" action="/admin/radiologie/store">
    <?= csrf_field() ?>
    <div class="row g-3">
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            ...
            <select name="type_examen" class="form-select" required>
                ...
                <option value="<?= $t ?>" <?= old('type_examen') == $t ? 'selected' : '' ?>><?= $t ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-6">
            <label class="form-label"><?= lang('Radiologie.region') ?></label>
            <input type="text" name="region" class="form-control" value="<?= esc(old('region')) ?>" placeholder="Thorax, Abdomen, Genou...">
        </div>
        <div class="col-md-4">
